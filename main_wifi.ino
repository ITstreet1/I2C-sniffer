...
//pin declarations and definitions
#define PIN_ISO_LOW 25      //low side of an ISO1540
#define PIN_ISO_HIGH 26     //high side of an ISO1540
#define PIN_STATUS_WIFI 16  //wifi status
#define PIN_STATUS_CONN 17  //connection status
#define PIN_STATUS_SEND 18  //send status
#define PIN_STATUS_ERROR 19 //error status
...
//v1.2
String msgToServer = "link to server";
...
void sendMsgOut(String argStr)
{
  ...
  if(WiFi.status()== WL_CONNECTED){
    digitalWrite(PIN_STATUS_CONN, HIGH);
      HTTPClient http;
      http.begin(argStr);
      ...
      if (httpResponseCode>0) {
        ...
        if(httpResponseCode != 200){
          digitalWrite(PIN_STATUS_ERROR, HIGH);
          }else{
            digitalWrite(PIN_STATUS_ERROR, LOW);
            }
        //String payload = http.getString();
        //Serial.println(payload);
        digitalWrite(PIN_STATUS_CONN, LOW);
...
{
    Serial.begin(115200);
    pinMode(PIN_STATUS_WIFI, OUTPUT);
    pinMode(PIN_STATUS_CONN, OUTPUT);
    pinMode(PIN_STATUS_SEND, OUTPUT);
    pinMode(PIN_STATUS_ERROR, OUTPUT);
    pinMode(PIN_ISO_LOW, OUTPUT);
    pinMode(PIN_ISO_HIGH, OUTPUT);
    digitalWrite(PIN_STATUS_WIFI, LOW);
    digitalWrite(PIN_STATUS_CONN, LOW);
    digitalWrite(PIN_STATUS_SEND, LOW);
    digitalWrite(PIN_STATUS_ERROR, LOW);
    digitalWrite(PIN_ISO_LOW, HIGH);
    digitalWrite(PIN_ISO_HIGH, LOW); 
    #ifdef I2CTEST
    ...
    delay(1000);
    digitalWrite(PIN_ISO_LOW, LOW);
    digitalWrite(PIN_ISO_HIGH, HIGH); 
...
void checkNetwork(){
  if(WiFi.status() == WL_CONNECTED){
    digitalWrite(PIN_STATUS_WIFI, HIGH);
    }else{
      digitalWrite(PIN_STATUS_WIFI, (millis() / 1000) % 2);
      }
}
